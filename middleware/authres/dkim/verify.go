...
type Verification struct {
	...
	// The Selector used to sign the message.
	Selector string

	// The short signature of the message, denoted as _b_.
	// 8 first chars of the signature.
	// eg given "b=HOoby/c>5zVFX7a48sFx...", Signature is "HOoby/c5"
	Signature string
...
func verify(h header, r io.Reader, sigField, sigValue string, options *VerifyOptions) (*Verification, error) {
	...
	if len(verif.Signature) > 8 {
		verif.Signature = verif.Signature[:8]
	}
