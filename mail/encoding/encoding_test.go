...
func TestEncodingMimeHeaderDecodeEnding(t *testing.T) {
	...
	str := envelope.MimeHeaderDecode("What about this one? =?ISO-8859-1?Q?Andr=E9?=")
	...
	str = envelope.MimeHeaderDecode("=?ISO-8859-1?Q?Andr=E9?= What about this one? =?ISO-8859-1?Q?Andr=E9?=")
	...
	str = envelope.MimeHeaderDecode("=?ISO-8859-1?B?Andr=E9?= What about this one? =?ISO-8859-1?Q?Andr=E9?=")
...
func TestEncodingMimeHeaderDecodeBad(t *testing.T) {
	...
	str := envelope.MimeHeaderDecode("=?ISO-8859-1?B?Andr=E9?=\tPirard <PIRARD@vm1.ulg.ac.be>")
...
func TestEncodingMimeHeaderDecodeNoSpace(t *testing.T) {
	// there is no space
	str := envelope.MimeHeaderDecode("A =?ISO-8859-1?Q?Andr=E9?=WORLD IN YOUR POCKET")
...
func TestEncodingMimeHeaderDecodeMulti(t *testing.T) {
	...
	str := envelope.MimeHeaderDecode("=?iso-2022-jp?B?GyRCIVpLXEZ8Om89fCFbPEIkT0lUOk5NUSROJU0lPyROSn0bKEI=?= =?iso-2022-jp?B?GyRCJCxCPyQkJEckORsoQg==?=")
